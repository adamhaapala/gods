...
public class WhitelistManager
{
	...
	public void load()
	{
		if (this.plugin.useWhitelist)
		{
			...
			{
				this.whiteListFile = new File(this.plugin.getDataFolder(),
						"whitelist.yml");
			}

			this.whiteList = YamlConfiguration
					.loadConfiguration(this.whiteListFile);

			this.plugin.log("Loaded " + this.whiteList.getKeys(false).size()
					+ " whitelisted Gods.");

			if (this.whiteList.getKeys(false).size() == 0)
			{
				this.whiteList.set("TheExampleGodName.MinPower",
						Integer.valueOf(0));
		...
		if (this.plugin.useBlacklist)
		{
			...
			{
				this.blackListFile = new File(this.plugin.getDataFolder(),
						"blacklist.yml");
			}

			this.blackList = YamlConfiguration
					.loadConfiguration(this.blackListFile);

			this.plugin.log("Loaded " + this.blackList.getKeys(false).size()
					+ " blacklisted Gods.");
	...
	{
		String name = whiteList.getString(godName);
	...
	{
		int power = whiteList.getInt(godName + ".MinPower");
